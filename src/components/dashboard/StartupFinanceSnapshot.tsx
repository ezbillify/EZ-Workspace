  ...
  return (
    <Card className="mb-6 overflow-hidden border-0 bg-[#08121f] p-0 text-white shadow-[0_20px_60px_rgba(8,18,31,0.35)]">
      <div className="border-b border-white/10 px-6 py-4 lg:px-7">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs text-slate-200/75">
        ...
        <div className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              ...
              <div key={label} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 backdrop-blur-sm">
                ...
                <p className="mt-1 text-xs text-slate-300/65">{note}</p>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
            <div className="mb-3 flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-100">Startup Position</p>
                <p className="text-xs text-slate-300/65">Built for an early-stage company, not a mature enterprise balance sheet.</p>
              </div>
            ...
            <div className="flex h-[210px] items-center justify-center">
              ...
              >
                <div className="flex h-28 w-28 flex-col items-center justify-center rounded-full bg-[#08121f] text-center">
                  ...
                  <p className="text-[11px] text-slate-300/65">margin</p>
                </div>
              </div>
            </div>
            <div className="-mt-2 text-center">
              <p className="text-xs text-slate-300/65">Configured profit margin for the current startup stage</p>
            </div>
          </div>

          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
            <div className="mb-2 text-sm font-medium text-slate-100">Profit vs Expense Mix</div>
            <div className="mt-5 space-y-4">
              {[
                ...
                <div key={label}>
                  ...
                  <p className="mt-1 text-xs text-slate-300/65">{formatCurrency(amount)}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="border-t border-white/10 px-6 py-5 lg:px-7">
        <div className="mb-3 text-sm font-medium text-slate-100">Revenue Allocation</div>
        <div className="grid gap-4 md:grid-cols-3">
          {allocationData.map((item) => (
            <div key={item.name} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-200/80">{item.name}</span>
